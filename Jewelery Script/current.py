...
print("end")
time.sleep(10000)
# so now we need to check the single tags and keep the markers as the same as the original list
delmimterArr = []
...
for i in range(len(markerArr)):
    ...
    if "-" in word:
        temp = word.split("-", 1)[0] 
        delmimterArr.append(temp)
    else:
        delmimterArr.append(word)
   
print(delmimterArr)
for word in delmimterArr:
    print(word)
